...
export default function DashboardLayout({
  ...
  if (!user) {
    // AuthProvider should handle redirect, this is a fallback or for explicit denial
    return (
       <div className="flex items-center justify-center h-screen bg-background">
        <p className="text-xl text-destructive">Access Denied. Redirecting to login...</p>
      </div>
    );
  }
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className={cn(
        ...
        <div className="flex h-16 items-center justify-center border-b border-border px-4">
          <Link href={user.isAdmin ? "/admin" : "/dashboard"} className="flex items-center gap-2 font-semibold text-lg text-primary">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-background">
          <motion.div
            ...
            className="max-w-full mx-auto" // Ensure content area can expand
          >
            {children}
          </motion.div>
        </main>
...
// Need to add a Link component if not already there
